...
1. **No argument** (`/tabtrack:recall`) — budgeted auto-load for the current project, last 14 days:
   ```bash
...
2. **Project name** (`/tabtrack:recall my-project`, or `all` for every project):
   ```bash
   python3 "${CLAUDE_PLUGIN_ROOT}/scripts/memo_recall.py" auto --project <name>
   ```

3. **Index only** (`/tabtrack:recall index`, optionally with a project and `--days N`): list entries one per line without loading their content:
   ```bash
...
4. **Specific entries** (`/tabtrack:recall general/2026-09-25#3,general/2026-09-25#5`): ids as printed by the index:
   ```bash
